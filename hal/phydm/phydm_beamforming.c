...
void beamforming_sw_timer_callback( void *function_context)
#else
void beamforming_sw_timer_callback(struct timer_list *t)
#endif
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	struct PHY_DM_STRUCT	*p_dm_odm = (struct PHY_DM_STRUCT *)function_context;
#else
	struct PHY_DM_STRUCT    *p_dm_odm = from_timer(p_dm_odm, t, beamforming_info.beamforming_timer);
#endif
	struct _ADAPTER	*adapter = p_dm_odm->adapter;
