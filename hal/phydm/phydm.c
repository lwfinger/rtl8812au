...
)
{
	...
	case	ODM_CMNINFO_EFUSE0X3D7:
		...
		break;
	case	ODM_CMNINFO_IQK_THERMAL_THRESHOLD:
		p_dm_odm->rf_calibrate_info.iqk_thermal_threshold = (u8)value;
		break;
	case	ODM_CMNINFO_LCK_THERMAL_THRESHOLD:
		p_dm_odm->rf_calibrate_info.lck_thermal_threshold = (u8)value;
		break;
...
#else
	timer_setup(&p_dm_odm->beamforming_info.beamforming_timer, beamforming_sw_timer_callback, 0);
#endif
#endif
}
...
/*<20161222, BB-David>D-CUT DC cancellation*/
void
...
{	
#if (defined(PHYDM_DC_CANCELLATION))
	u32		offset_i_hex = 0;
	u32		offset_q_hex = 0;
	u32		reg_value32 = 0;
	...
	if (p_dm_odm->support_ic_type & (ODM_RTL8188F | ODM_RTL8710B)) {
		offset_i_hex = (reg_value32 & 0xffc0000) >> 18;
		offset_q_hex = (reg_value32 & 0x3ff00) >> 8;

		/*Before filling into registers, offset should be multiplexed (-1)*/
		offset_i_hex = (offset_i_hex >= 0x200) ? (0x400 - offset_i_hex) : (0x1ff - offset_i_hex);
		offset_q_hex = (offset_q_hex >= 0x200) ? (0x400 - offset_q_hex) : (0x1ff - offset_q_hex);

		odm_set_bb_reg(p_dm_odm, 0x950, 0x1ff, offset_i_hex);
		odm_set_bb_reg(p_dm_odm, 0x950, 0x1ff0000, offset_q_hex);
	} else if (p_dm_odm->support_ic_type & (ODM_RTL8821C | ODM_RTL8822B)) {
		offset_i_hex = (reg_value32 & 0xffc00) >> 10;
		offset_q_hex = reg_value32 & 0x3ff;

		/*Before filling into registers, offset should be multiplexed (-1)*/
		offset_i_hex = 0x400 - offset_i_hex;
		offset_q_hex = 0x400 - offset_q_hex;

		odm_set_bb_reg(p_dm_odm, 0xc10, 0x3c000000, ((0x3c0 & offset_i_hex) >> 6));
		odm_set_bb_reg(p_dm_odm, 0xc10, 0xfc00, (0x3f & offset_i_hex));
		odm_set_bb_reg(p_dm_odm, 0xc14, 0x3c000000, ((0x3c0 & offset_q_hex) >> 6));
		odm_set_bb_reg(p_dm_odm, 0xc14, 0xfc00, (0x3f & offset_q_hex));
	}
#endif
}
