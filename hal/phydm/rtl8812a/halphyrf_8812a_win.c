...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("===>odm_tx_pwr_track_set_pwr8812a\n"));
	...
	if (method == BBSWING) {
		...
		if (rf_path == ODM_RF_PATH_A) {
			...
			final_bb_swing_idx[ODM_RF_PATH_B] = (p_rf_calibrate_info->OFDM_index[ODM_RF_PATH_B] > pwr_tracking_limit) ? pwr_tracking_limit : p_rf_calibrate_info->OFDM_index[ODM_RF_PATH_B];
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("p_rf_calibrate_info->OFDM_index[ODM_RF_PATH_B]=%d, p_dm_odm->RealBbSwingIdx[ODM_RF_PATH_B]=%d\n",
				p_rf_calibrate_info->OFDM_index[ODM_RF_PATH_B], final_bb_swing_idx[ODM_RF_PATH_B]));

			odm_set_bb_reg(p_dm_odm, REG_B_TX_SCALE_JAGUAR, 0xFFE00000, tx_scaling_table_jaguar[final_bb_swing_idx[ODM_RF_PATH_B]]);
		}
	}
		...
		if (rf_path == ODM_RF_PATH_B) {
			if (final_ofdm_swing_index > pwr_tracking_limit) {  /* BBSwing higher then Limit */
				...
				odm_set_bb_reg(p_dm_odm, REG_B_TX_SCALE_JAGUAR, 0xFFE00000, tx_scaling_table_jaguar[pwr_tracking_limit]);

				p_rf_calibrate_info->modify_tx_agc_flag_path_b = true;

				PHY_SetTxPowerLevelByPath(adapter, *p_dm_odm->p_channel, ODM_RF_PATH_B);

				ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_B Over BBSwing Limit, pwr_tracking_limit = %d, Remnant tx_agc value = %d\n", pwr_tracking_limit, p_rf_calibrate_info->remnant_ofdm_swing_idx[rf_path]));
			} else if (final_ofdm_swing_index < 0) {
				p_rf_calibrate_info->remnant_ofdm_swing_idx[rf_path] = final_ofdm_swing_index;

				odm_set_bb_reg(p_dm_odm, REG_B_TX_SCALE_JAGUAR, 0xFFE00000, tx_scaling_table_jaguar[0]);

				p_rf_calibrate_info->modify_tx_agc_flag_path_b = true;

				PHY_SetTxPowerLevelByPath(adapter, *p_dm_odm->p_channel, ODM_RF_PATH_B);

				ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_B Lower then BBSwing lower bound  0, Remnant tx_agc value = %d\n", p_rf_calibrate_info->remnant_ofdm_swing_idx[rf_path]));
			} else {
				odm_set_bb_reg(p_dm_odm, REG_B_TX_SCALE_JAGUAR, 0xFFE00000, tx_scaling_table_jaguar[final_ofdm_swing_index]);

				ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_B Compensate with BBSwing, final_ofdm_swing_index = %d\n", final_ofdm_swing_index));
				...
				if (p_rf_calibrate_info->modify_tx_agc_flag_path_b) { /* If tx_agc has changed, reset tx_agc again */
					...
					ODM_RT_TRACE(p_dm_odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, ("******Path_B p_dm_odm->Modify_TxAGC_Flag = false\n"));
				}
			}
		}
...
)
{
	...
	case ODM_RF_PATH_A:
	{
		...
		if (RX_X >> 1 >= 0x112 || (RX_Y >> 1 >= 0x12 && RX_Y >> 1 <= 0x3ee)) {
			...
			odm_set_bb_reg(p_dm_odm, 0xc10, 0x03ff0000, 0);
	...
	case ODM_RF_PATH_B:
	{
		odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
		if (RX_X >> 1 >= 0x112 || (RX_Y >> 1 >= 0x12 && RX_Y >> 1 <= 0x3ee)) {
			odm_set_bb_reg(p_dm_odm, 0xe10, 0x000003ff, 0x100);
			odm_set_bb_reg(p_dm_odm, 0xe10, 0x03ff0000, 0);
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX_X = %x;;RX_Y = %x ====>fill to IQC\n", RX_X >> 1 & 0x000003ff, RX_Y >> 1 & 0x000003ff));
		} else {
...
)
{
	...
	case ODM_RF_PATH_A:
	{
		...
		odm_set_bb_reg(p_dm_odm, 0xc90, BIT(7), 0x1);
		odm_set_bb_reg(p_dm_odm, 0xcc4, BIT(18), 0x1);
		if (!p_dm_odm->dpk_done)
			odm_set_bb_reg(p_dm_odm, 0xcc4, BIT(29), 0x1);
		odm_set_bb_reg(p_dm_odm, 0xcc8, BIT(29), 0x1);
	...
	{
		odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
		odm_set_bb_reg(p_dm_odm, 0xe90, BIT(7), 0x1);
		odm_set_bb_reg(p_dm_odm, 0xec4, BIT(18), 0x1);
		if (!p_dm_odm->dpk_done)
			odm_set_bb_reg(p_dm_odm, 0xec4, BIT(29), 0x1);
		odm_set_bb_reg(p_dm_odm, 0xec8, BIT(29), 0x1);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("band_width = %d, ext_pa_5g = %d, ExtPA2G = %d\n", *p_dm_odm->p_band_width, p_dm_odm->ext_pa_5g, p_dm_odm->ext_pa));
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("Interface = %d, RFE_Type = %d\n", p_dm_odm->support_interface, p_dm_odm->rfe_type));
	...
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0xef, RFREGOFFSETMASK, 0x80002);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x30, RFREGOFFSETMASK, 0x20000);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x31, RFREGOFFSETMASK, 0x3fffd);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x32, RFREGOFFSETMASK, 0xfe83f);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x65, RFREGOFFSETMASK, 0x931d5);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x8a001);
	/* ====== path B TX IQK RF setting ====== */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0xef, RFREGOFFSETMASK, 0x80002);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x30, RFREGOFFSETMASK, 0x20000);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x31, RFREGOFFSETMASK, 0x3fffd);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x32, RFREGOFFSETMASK, 0xfe83f);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x65, RFREGOFFSETMASK, 0x931d5);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x8a001);
	...
	if (p_dm_odm->ext_pa_5g) {
		if (p_dm_odm->rfe_type == 1) {
			...
			odm_write_4byte(p_dm_odm, 0xe88, 0x821403f7);
		}
	} else {
	...
	odm_write_4byte(p_dm_odm, 0xe80, 0x18008c10);/* TX_Tone_idx[9:0], TxK_Mask[29] TX_Tone = 16 */
	odm_write_4byte(p_dm_odm, 0xe84, 0x38008c10);/* RX_Tone_idx[9:0], RxK_Mask[29] */
	...
	while (1) {
		...
		odm_write_4byte(p_dm_odm, 0xeb8, 0x00100000);/* cb8[20] 將 SI/PI 使用權切給 iqk_dpk module */
		...
		while (1) {
			if (!TX0_finish)
				IQK0_ready = (boolean) odm_get_bb_reg(p_dm_odm, 0xd00, BIT(10));
			if (!TX1_finish)
				IQK1_ready = (boolean) odm_get_bb_reg(p_dm_odm, 0xd40, BIT(10));
		...
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("TX delay_count = %d\n", delay_count));
		if (delay_count < 20) {							/* If 20ms No Result, then cal_retry++ */
			/* ============TXIQK Check============== */
			TX0_fail = (boolean) odm_get_bb_reg(p_dm_odm, 0xd00, BIT(12));
			TX1_fail = (boolean) odm_get_bb_reg(p_dm_odm, 0xd40, BIT(12));
			if (!(TX0_fail || TX0_finish)) {
				odm_write_4byte(p_dm_odm, 0xcb8, 0x02000000);
				TX_IQC_temp[tx0_average][0] = odm_get_bb_reg(p_dm_odm, 0xd00, 0x07ff0000) << 21;
				odm_write_4byte(p_dm_odm, 0xcb8, 0x04000000);
				TX_IQC_temp[tx0_average][1] = odm_get_bb_reg(p_dm_odm, 0xd00, 0x07ff0000) << 21;
			...
			if (!(TX1_fail || TX1_finish)) {
				...
				TX_IQC_temp[tx1_average][2] = odm_get_bb_reg(p_dm_odm, 0xd40, 0x07ff0000) << 21;
				odm_write_4byte(p_dm_odm, 0xeb8, 0x04000000);
				TX_IQC_temp[tx1_average][3] = odm_get_bb_reg(p_dm_odm, 0xd40, 0x07ff0000) << 21;
			...
			cal0_retry++;
			cal1_retry++;
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay 20ms TX IQK Not Ready!!!!!\n"));
			if (cal0_retry == 10)
				break;
		}
		if (tx0_average >= 2) {
				...
				for (ii = i + 1; ii < tx0_average; ii++) {
					...
					if (dx < 4 && dx > -4) {
						...
						if (dy < 4 && dy > -4) {
							...
							ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("TXA_X = %x;;TXA_Y = %x\n", TX_IQC[0] & 0x000007ff, TX_IQC[1] & 0x000007ff));
							TX0_finish = true;
						}
					}
				}
				...
				for (ii = i + 1; ii < tx1_average; ii++) {
					...
					if (dx < 4 && dx > -4) {
						...
						if (dy < 4 && dy > -4) {
							...
							ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("TXB_X = %x;;TXB_Y = %x\n", TX_IQC[2] & 0x000007ff, TX_IQC[3] & 0x000007ff));
							TX1_finish = true;
						}
					}
				}
		...
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("tx0_average = %d, tx1_average = %d\n", tx0_average, tx1_average));
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("TX0_finish = %d, TX1_finish = %d\n", TX0_finish, TX1_finish));
	...
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x58, 0x7fe00, odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8, 0xffc00)); /* Load LOK */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x58, 0x7fe00, odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8, 0xffc00)); /* Load LOK */
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
	...
	if (TX0_finish) {
		...
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x30, RFREGOFFSETMASK, 0x30000);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x31, RFREGOFFSETMASK, 0x3f7ff);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x32, RFREGOFFSETMASK, 0xfe7bf);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x88001);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x65, RFREGOFFSETMASK, 0x931d1);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0xef, RFREGOFFSETMASK, 0x00000);
	}
	if (TX1_finish) {
		/* ====== path B RX IQK RF setting====== */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0xef, RFREGOFFSETMASK, 0x80000);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x30, RFREGOFFSETMASK, 0x30000);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x31, RFREGOFFSETMASK, 0x3f7ff);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x32, RFREGOFFSETMASK, 0xfe7bf);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x88001);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x65, RFREGOFFSETMASK, 0x931d1);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0xef, RFREGOFFSETMASK, 0x00000);
	}
	...
	if (p_dm_odm->support_interface == ODM_ITRF_PCIE)
		...
		odm_write_4byte(p_dm_odm, 0x984, 0x0046a890);
	/* odm_write_4byte(p_dm_odm, 0x984, 0x0046a890); */
	if (p_dm_odm->rfe_type == 1) {
		odm_write_4byte(p_dm_odm, 0xcb0, 0x77777717);
		odm_write_4byte(p_dm_odm, 0xcb4, 0x00000077);
		odm_write_4byte(p_dm_odm, 0xeb0, 0x77777717);
		odm_write_4byte(p_dm_odm, 0xeb4, 0x00000077);
	} else {
		odm_write_4byte(p_dm_odm, 0xcb0, 0x77777717);
		odm_write_4byte(p_dm_odm, 0xcb4, 0x02000077);
		odm_write_4byte(p_dm_odm, 0xeb0, 0x77777717);
		odm_write_4byte(p_dm_odm, 0xeb4, 0x02000077);
	}
	...
	if (TX0_finish) {
		...
		odm_write_4byte(p_dm_odm, 0xc88, 0x82140119);
	}
	if (TX1_finish) {
		odm_write_4byte(p_dm_odm, 0xe80, 0x38008c10);/* TX_Tone_idx[9:0], TxK_Mask[29] TX_Tone = 16 */
		odm_write_4byte(p_dm_odm, 0xe84, 0x18008c10);/* RX_Tone_idx[9:0], RxK_Mask[29] */
		odm_write_4byte(p_dm_odm, 0xe88, 0x82140119);
	}
	cal0_retry = 0;
	cal1_retry = 0;
	while (1) {
		...
		odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
		if (TX0_finish) {
			odm_set_bb_reg(p_dm_odm, 0x978, 0x03FF8000, (TX_IQC[0]) & 0x000007ff);
			odm_set_bb_reg(p_dm_odm, 0x978, 0x000007FF, (TX_IQC[1]) & 0x000007ff);
			odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
			if (p_dm_odm->rfe_type == 1)
				odm_write_4byte(p_dm_odm, 0xc8c, 0x28161500);
			else
				odm_write_4byte(p_dm_odm, 0xc8c, 0x28160cc0);
			odm_write_4byte(p_dm_odm, 0xcb8, 0x00300000);/* cb8[20] 將 SI/PI 使用權切給 iqk_dpk module */
		...
		if (TX1_finish) {
			odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
			odm_set_bb_reg(p_dm_odm, 0x978, 0x03FF8000, (TX_IQC[2]) & 0x000007ff);
			odm_set_bb_reg(p_dm_odm, 0x978, 0x000007FF, (TX_IQC[3]) & 0x000007ff);
			odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
			if (p_dm_odm->rfe_type == 1)
				odm_write_4byte(p_dm_odm, 0xe8c, 0x28161500);
			else
				odm_write_4byte(p_dm_odm, 0xe8c, 0x28160ca0);
			odm_write_4byte(p_dm_odm, 0xeb8, 0x00300000);/* cb8[20] 將 SI/PI 使用權切給 iqk_dpk module */
			odm_write_4byte(p_dm_odm, 0xeb8, 0x00100000);/* cb8[20] 將 SI/PI 使用權切給 iqk_dpk module */
			ODM_delay_ms(5); /* delay 5ms */
			odm_write_4byte(p_dm_odm, 0xe8c, 0x3c000000);
			odm_write_4byte(p_dm_odm, 0xeb8, 0x00000000);
		...
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX delay_count = %d\n", delay_count));
		if (delay_count < 20) {	/* If 20ms No Result, then cal_retry++ */
			/* ============RXIQK Check============== */
			RX0_fail = (boolean) odm_get_bb_reg(p_dm_odm, 0xd00, BIT(11));
			RX1_fail = (boolean) odm_get_bb_reg(p_dm_odm, 0xd40, BIT(11));
			if (!(RX0_fail || RX0_finish) && TX0_finish) {
				odm_write_4byte(p_dm_odm, 0xcb8, 0x06000000);
				RX_IQC_temp[rx0_average][0] = odm_get_bb_reg(p_dm_odm, 0xd00, 0x07ff0000) << 21;
				odm_write_4byte(p_dm_odm, 0xcb8, 0x08000000);
				RX_IQC_temp[rx0_average][1] = odm_get_bb_reg(p_dm_odm, 0xd00, 0x07ff0000) << 21;
				...
				*/
				...
				ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("1. RXA_cal_retry = %d\n", cal0_retry));
				cal0_retry++;
				if (cal0_retry == 10)
					break;
			}
			if (!(RX1_fail || RX1_finish) && TX1_finish) {
				...
				RX_IQC_temp[rx1_average][2] = odm_get_bb_reg(p_dm_odm, 0xd40, 0x07ff0000) << 21;
				odm_write_4byte(p_dm_odm, 0xeb8, 0x08000000);
				RX_IQC_temp[rx1_average][3] = odm_get_bb_reg(p_dm_odm, 0xd40, 0x07ff0000) << 21;
			...
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("2. RXA_cal_retry = %d\n", cal0_retry));
			cal0_retry++;
			cal1_retry++;
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("delay 20ms RX IQK Not Ready!!!!!\n"));
			if (cal0_retry == 10)
				break;
		}
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("3. RXA_cal_retry = %d\n", cal0_retry));
		if (rx0_average >= 2) {
		...
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("rx0_average = %d, rx1_average = %d\n", rx0_average, rx1_average));
		ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("RX0_finish = %d, RX1_finish = %d\n", RX0_finish, RX1_finish));
	...
	if (TX0_finish)
		_iqk_tx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_A, TX_IQC[0], TX_IQC[1]);
	else
		_iqk_tx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_A, 0x200, 0x0);



	if (RX0_finish)
		_iqk_rx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_A, RX_IQC[0], RX_IQC[1]);
	else
		_iqk_rx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_A, 0x200, 0x0);

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("========Path_B =======\n"));

	if (TX1_finish)
		_iqk_tx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_B, TX_IQC[2], TX_IQC[3]);
	else
		_iqk_tx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_B, 0x200, 0x0);



	if (RX1_finish)
		_iqk_rx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_B, RX_IQC[2], RX_IQC[3]);
	else
		_iqk_rx_fill_iqc_8812a(p_dm_odm, ODM_RF_PATH_B, 0x200, 0x0);
...
)
{
	...
	reg_c1b8 = odm_read_4byte(p_dm_odm, 0xcb8);
	reg_e1b8 = odm_read_4byte(p_dm_odm, 0xeb8);
	...
	_iqk_tx_8812a(p_dm_odm, chnl_idx);
	...
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1);
	odm_write_4byte(p_dm_odm, 0xcb8, reg_c1b8);
	odm_write_4byte(p_dm_odm, 0xeb8, reg_e1b8);
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0);
...
)
{
	...
	u32	reg0x914 = odm_read_4byte(p_dm_odm, REG_SINGLE_TONE_CONT_TX_JAGUAR);;
	...
	if ((reg0x914 & 0x70000) != 0)	/* If contTx, disable all continuous TX. 0x914[18:16] */
		...
		;/* odm_write_4byte(p_dm_odm, REG_SINGLE_TONE_CONT_TX_JAGUAR, reg0x914 & (~0x70000)); */
	...
	if ((reg0x914 & 70000) != 0) {	/* Deal with contisuous TX case, 0x914[18:16] */
		/* <20121121, Kordan> A workaround: If we set 0x914[18:16] as zero, BB turns off ContTx */
		/* until another packet comes in. To avoid ContTx being turned off, we skip this step. */
		/* odm_write_4byte(p_dm_odm, REG_SINGLE_TONE_CONT_TX_JAGUAR, reg0x914); */
	} else /* Deal with Packet TX case */
		odm_write_1byte(p_dm_odm, REG_TXPAUSE_8812A, 0x00);

	/* Recover channel number */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, RF_CHNLBW, RFREGOFFSETMASK, lc_cal);

	/*
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, RF_AC, RFREGOFFSETMASK, rf_amode);
	if(is2T)
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, RF_AC, RFREGOFFSETMASK, rf_bmode);
		*/
...
)
{
	...
	u8 chnl_idx = get_right_chnl_place_for_iqk(channel);
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x1); /* [31] = 1 --> Page C1 */
	odm_set_bb_reg(p_dm_odm, 0xccc, 0x000007ff, p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][0] & 0x7ff);
	odm_set_bb_reg(p_dm_odm, 0xcd4, 0x000007ff, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][0] & 0x7ff0000) >> 16);
	odm_set_bb_reg(p_dm_odm, 0xecc, 0x000007ff, p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][2] & 0x7ff);
	odm_set_bb_reg(p_dm_odm, 0xed4, 0x000007ff, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][2] & 0x7ff0000) >> 16);

	if (*p_dm_odm->p_band_width != 2) {
		odm_write_4byte(p_dm_odm, 0xce8, 0x0);
		odm_write_4byte(p_dm_odm, 0xee8, 0x0);
	} else {
		odm_write_4byte(p_dm_odm, 0xce8, p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][4]);
		odm_write_4byte(p_dm_odm, 0xee8, p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][5]);
	}
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
	odm_set_bb_reg(p_dm_odm, 0xc10, 0x000003ff, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][1] & 0x7ff0000) >> 17);
	odm_set_bb_reg(p_dm_odm, 0xc10, 0x03ff0000, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][1] & 0x7ff) >> 1);
	odm_set_bb_reg(p_dm_odm, 0xe10, 0x000003ff, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][3] & 0x7ff0000) >> 17);
	odm_set_bb_reg(p_dm_odm, 0xe10, 0x03ff0000, (p_rf_calibrate_info->iqk_matrix_reg_setting[chnl_idx].value[*p_dm_odm->p_band_width][3] & 0x7ff) >> 1);
...
)
{
	...
	odm_set_bb_reg(p_dm_odm, 0xecc, 0x000007ff, 0x0);
	odm_set_bb_reg(p_dm_odm, 0xed4, 0x000007ff, 0x200);
	odm_write_4byte(p_dm_odm, 0xce8, 0x0);
	odm_write_4byte(p_dm_odm, 0xee8, 0x0);
	odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
	odm_set_bb_reg(p_dm_odm, 0xc10, 0x000003ff, 0x100);
...
)
{
	...
	iqk_cmd[1] = buf1 | buf2;
	iqk_cmd[2] =  p_dm_odm->ext_pa_5g | p_dm_odm->ext_pa << 1 | p_dm_odm->support_interface << 2 | p_dm_odm->rfe_type << 5;

	RT_TRACE(COMP_MP, DBG_LOUD, ("== FW IQK Start ==\n"));
	p_rf_calibrate_info->iqk_start_time = 0;
	p_rf_calibrate_info->iqk_start_time = odm_get_current_time(p_dm_odm);
	RT_TRACE(COMP_MP, DBG_LOUD, ("== start_time: %u\n", p_rf_calibrate_info->iqk_start_time));
...
)
{
	...
	if (odm_check_power_status(p_adapter) == false)
		return;
#endif
	/* p_mgnt_info->reg_iqk_fw_offload = (u8)odm_get_bb_reg( p_dm_odm, 0x444, BIT8); */
#if MP_DRIVER == 1
...
#endif
	{
		...
		if (p_mgnt_info->RegIQKFWOffload) {
			if (!p_rf_calibrate_info->is_iqk_in_progress) {
				...
				_phy_iq_calibrate_by_fw_8812a(p_dm_odm);
			} else {
		...
		/* 3 == Driver IQK == */
			...
			if (!p_rf_calibrate_info->is_iqk_in_progress) {
				...
				_phy_iq_calibrate_8812a(p_dm_odm, *p_dm_odm->p_channel);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> phy_lc_calibrate_8812a\n"));

#if (MP_DRIVER == 1)
	_phy_lc_calibrate_8812a(p_dm_odm, true);
#endif
...
)
{
	...
	if (IS_HARDWARE_TYPE_8821(p_adapter)) {
		if (is_main)
			odm_set_bb_reg(p_dm_odm, REG_A_RFE_PINMUX_JAGUAR + 4, BIT(29) | BIT28, 0x1);	/* Main */
		else
			odm_set_bb_reg(p_dm_odm, REG_A_RFE_PINMUX_JAGUAR + 4, BIT(29) | BIT28, 0x2);	/* Aux */
	} else if (p_dm_odm->support_ic_type & ODM_RTL8812) {
		if (p_dm_odm->rfe_type == 5) {
			if (is_main) {
				/* WiFi */
				odm_set_bb_reg(p_dm_odm, REG_ANTSEL_SW_JAGUAR, BIT(1) | BIT(0), 0x2);
				odm_set_bb_reg(p_dm_odm, REG_ANTSEL_SW_JAGUAR, BIT(9) | BIT(8), 0x3);
			} else {
				/* BT */
				odm_set_bb_reg(p_dm_odm, REG_ANTSEL_SW_JAGUAR, BIT(1) | BIT(0), 0x1);
				odm_set_bb_reg(p_dm_odm, REG_ANTSEL_SW_JAGUAR, BIT(9) | BIT(8), 0x3);
			}
...
)
{
	...
	for (i = 0; i < number; i++)
		backup_reg_data[i] = odm_read_4byte(p_dm_odm, backup_reg_addr[i]);
}
void
...
)
{
	...
	for (i = 0; i < number; i++)
		odm_write_4byte(p_dm_odm, backup_reg_addr[i], backup_reg_data[i]);
}
...
)
{
	...
	for (i = 0; i < number; i++)
		backup_reg_data[i] = odm_get_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)rf_path, backup_reg_addr[i], RFREGOFFSETMASK);
}
...
)
{
	...
	for (i = 0; i < number; i++)
		odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)rf_path, backup_reg_addr[i], RFREGOFFSETMASK, backup_reg_data[i]);
}
...
)
{
	...
	* 舉例5, d00[10:0] = h'7ff, 經過 hex2dec(h'7ff) = -1, */
	u32 reg0xd00_26_16 = odm_get_bb_reg(p_dm_odm, 0xD00, 0x7FF0000);
	u32 reg0xd00_10_0  = odm_get_bb_reg(p_dm_odm, 0xD00, 0x7FF);

	s32 tmp_lb_gain = _sign(reg0xd00_26_16) * _sign(reg0xd00_26_16) + _sign(reg0xd00_10_0) * _sign(reg0xd00_10_0);

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("<=== _compute_loop_back_gain_path_a, tmp_lb_gain = 0x%X\n", tmp_lb_gain));
...
)
{
	...
	u32 rf0x64_orig = odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x64, 0x7);
	u32 rf0x64_new = rf0x64_orig;

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _FineTuneLoopBackGain\n"));

	do {
		/* RF setting */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x00, RFREGOFFSETMASK, dpk_tx_agc);
		odm_write_4byte(p_dm_odm, 0x82c, 0x802083dd);
		/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
		odm_write_4byte(p_dm_odm, 0xc90, 0x0101f018);

		/* one shot */
		odm_write_4byte(p_dm_odm, 0xcc8, 0x800c5599);
		odm_write_4byte(p_dm_odm, 0xcc8, 0x000c5599);
		/* delay 50 ms,讓 delay 時間長一點, 確定 PA Scan function 有做完 */
		delay_ms(50);

		/* reg82c[31] = 0 --> 切到 page C */
		odm_write_4byte(p_dm_odm, 0x82c, 0x002083dd);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x00, RFREGOFFSETMASK, 0x33d8d);

		tmp_lb_gain = _compute_loop_back_gain_path_a(p_dm_odm);
		...
		if ((rf0x64_new == (BIT(2) | BIT(1) | BIT(0))) || (rf0x64_new == 0)) {
			...
			if (tmp_lb_gain < 263390) {/* fine tune loopback path gain: newReg64[2:0] = reg64[2:0] - 3b'001 */
				rf0x64_new -= 1;
				odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x64, 0x7, rf0x64_new);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _DPK_Init\n"));
	...
	odm_write_4byte(p_dm_odm, 0xcb4, 0x01000077);

	/* hardware 3-wire off */
	odm_write_4byte(p_dm_odm, 0xc00, 0x00000004);

	/* CCA off */
	odm_write_4byte(p_dm_odm, 0x838, 0x16C89B4c);

	/* 90c[15]: dac fifo reset by CSWU */
	odm_write_4byte(p_dm_odm, 0x90c, 0x00008000);

	/* r_gothrough_iqkdpk */
	odm_write_4byte(p_dm_odm, 0xc94, 0x0100005D);
	...
	/* tx_amp */
	odm_write_4byte(p_dm_odm, 0xc98, 0x41382e21);
	odm_write_4byte(p_dm_odm, 0xc9c, 0x5b554f48);
	odm_write_4byte(p_dm_odm, 0xca0, 0x6f6b6661);
	odm_write_4byte(p_dm_odm, 0xca4, 0x817d7874);
	odm_write_4byte(p_dm_odm, 0xca8, 0x908c8884);
	odm_write_4byte(p_dm_odm, 0xcac, 0x9d9a9793);
	odm_write_4byte(p_dm_odm, 0xcb0, 0xaaa7a4a1);
	odm_write_4byte(p_dm_odm, 0xcb4, 0xb6b3b0ad);

	/* tx_inverse */
	odm_write_4byte(p_dm_odm, 0xc40, 0x02ce03e9);
	odm_write_4byte(p_dm_odm, 0xc44, 0x01fd0249);
	odm_write_4byte(p_dm_odm, 0xc48, 0x01a101c9);
	odm_write_4byte(p_dm_odm, 0xc4c, 0x016a0181);
	odm_write_4byte(p_dm_odm, 0xc50, 0x01430155);
	odm_write_4byte(p_dm_odm, 0xc54, 0x01270135);
	odm_write_4byte(p_dm_odm, 0xc58, 0x0112011c);
	odm_write_4byte(p_dm_odm, 0xc5c, 0x01000108);
	odm_write_4byte(p_dm_odm, 0xc60, 0x00f100f8);
	odm_write_4byte(p_dm_odm, 0xc64, 0x00e500eb);
	odm_write_4byte(p_dm_odm, 0xc68, 0x00db00e0);
	odm_write_4byte(p_dm_odm, 0xc6c, 0x00d100d5);
	odm_write_4byte(p_dm_odm, 0xc70, 0x00c900cd);
	odm_write_4byte(p_dm_odm, 0xc74, 0x00c200c5);
	odm_write_4byte(p_dm_odm, 0xc78, 0x00bb00be);
	odm_write_4byte(p_dm_odm, 0xc7c, 0x00b500b8);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _DPK_AdjustRFGain\n"));

	/* RF setting */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x00, RFREGOFFSETMASK, 0x50bfc);
	/* Attn: A mode @ reg64[2:0], G mode @ reg56 */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x64, RFREGOFFSETMASK, 0x19aac);
	...
	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xc94, 0xf76c9f84);
	odm_write_4byte(p_dm_odm, 0xcc8, 0x000c5599);
	odm_write_4byte(p_dm_odm, 0xcc4, 0x11838000);
	odm_set_bb_reg(p_dm_odm, 0xcd4, 0xFFF000, 0x100);  /* 將cd4[23:12] 改成 h'100, 其它位元請保留原值不要寫到
     * 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xc90, 0x0101f018);
	...
	odm_write_4byte(p_dm_odm, 0xcb8, 0x09000000);
	...
	if (tmp_lb_gain < 263390) {
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x8e001);
		_fine_tune_loop_back_gain_path_a(p_dm_odm, 0x50bfc);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _DPK_GainLossToFindTxAGC\n"));
	...
	odm_write_4byte(p_dm_odm, 0xc20, 0x00002000);

	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xc94, 0xf76c9f84);
	odm_write_4byte(p_dm_odm, 0xcc8, 0x000c5599);
	odm_write_4byte(p_dm_odm, 0xcc4, 0x148b8000);

	/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xc90, 0x0401f018);
	...
	/*  */
	reg0xd00 = odm_read_4byte(p_dm_odm, 0xd00);
	switch (odm_read_4byte(p_dm_odm, 0xd00) & (BIT(3) | BIT(2) | BIT(1) | BIT(0))) {
	case 0x0:
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _DPK_AdjustRFGainByFoundTxAGC\n"));

	/* RF setting, 重新設定RF reg00, 舉例用DPK Phase 2得到的 d00[3:0] = 0x6 ' TX AGC= 0x18 ' RF reg00[4:0] = 0x18 */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x00, RFREGOFFSETMASK, p_rf_calibrate_info->dpk_tx_agc);
	/* Attn: A mode @ reg64[2:0], G mode @ reg56 */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x64, RFREGOFFSETMASK, 0x19aac);
	/* PGA gain: RF reg8f[14:13] */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x8a001);
	...
	if (tmp_lb_gain < 263390) {
		/* coarse tune loopback gain by RF reg8f[14:13] = 2b'11 */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x8e001);
		_fine_tune_loop_back_gain_path_a(p_dm_odm, p_rf_calibrate_info->dpk_tx_agc);
	} else if (tmp_lb_gain > 661607) {
		/* coarse tune loopback gain by RF reg8f[14:13] = 2b'00 */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x8f, RFREGOFFSETMASK, 0x88001);
		_fine_tune_loop_back_gain_path_a(p_dm_odm, p_rf_calibrate_info->dpk_tx_agc);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _DPK_DoAutoDPK\n"));
	...
	odm_write_4byte(p_dm_odm, 0xc20, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc24, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc28, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc2c, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc30, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc34, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc38, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc3c, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc40, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc44, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc48, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xc4c, 0x3c3c3c3c);
	...
	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xc94, 0xf76C9f84);
	odm_write_4byte(p_dm_odm, 0xcc8, 0x400C5599);
	odm_write_4byte(p_dm_odm, 0xcc4, 0x11938080);    /* 0xcc4[9:4]= DPk fail threshold */

	if (36 <= *(p_dm_odm->p_channel) && *(p_dm_odm->p_channel) <= 53) /* Channelchannel at low band) */
		/* r_agc */
		odm_write_4byte(p_dm_odm, 0xcbc, 0x00022a1f);
	else
		/* r_agc */
		odm_write_4byte(p_dm_odm, 0xcbc, 0x00009dbf);

	/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xc90, 0x0101f018); /* TODO: 0xC90(rA_LSSIWrite_Jaguar) can not be overwritten. */

	/* one shot */
	odm_write_4byte(p_dm_odm, 0xcc8, 0xc00c5599);
	odm_write_4byte(p_dm_odm, 0xcc8, 0x400c5599);
	/* delay 50 ms,讓 delay 時間長一點, 確定 PA Scan function 有做完 */
	delay_ms(50);
	...
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x42, BIT(17), 1);
	p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_A] = odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x42, 0xFC00);					/* 讀出42[15:10] 值並存到變數TMeter */
	dbg_print("p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_A] = 0x%X\n", p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_A]);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x00, RFREGOFFSETMASK, 0x33D8D);
	...
	/* read back */
	odm_write_4byte(p_dm_odm, 0xc90, 0x0201f01f);
	odm_write_4byte(p_dm_odm, 0xcb8, 0x0c000000);
...
)
{
	...
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _dpk_enable_dp\n"));
	...
	odm_write_4byte(p_dm_odm, 0xc90, 0x0000f098);
	odm_write_4byte(p_dm_odm, 0xc94, 0x776d9f84);
	odm_write_4byte(p_dm_odm, 0xcc8, 0x20000000);
	odm_write_4byte(p_dm_odm, 0xc8c, 0x3c000000);
	/* r_bnd */
	odm_write_4byte(p_dm_odm, 0xcb8, 0x000fffff);

	if (p_rf_calibrate_info->is_dpk_fail) {
		/* cc4[29] = 1 (bypass DP LUT) */
		odm_write_4byte(p_dm_odm, 0xc98, 0x40004000);
		odm_write_4byte(p_dm_odm, 0xcc4, 0x28840000);
	} else {
		odm_write_4byte(p_dm_odm, 0xc98, p_rf_calibrate_info->dpk_gain);
		odm_write_4byte(p_dm_odm, 0xcc4, 0x08840000);

		/* PWSF */
		/* 寫PWSF table in 1st SRAM for PA = 11 use */
		odm_write_4byte(p_dm_odm, 0xc20, 0x00000800);
...

		{
			...
			for (i = start_idx, j = 0; (center_addr - j + 1) >= 1; i -= 2, j++) {
				...
				odm_write_4byte(p_dm_odm, 0xce4, value);
				odm_write_1byte(p_dm_odm, 0xce4, 0x0);		/* write disable */
			}
...
)
{
	...
	_dpk_mac_bb_backup_path_a(p_dm_odm, backup_mac_bb_reg_addrs, backup_mac_bb_reg_data, sizeof(backup_mac_bb_reg_addrs) / sizeof(u32));
	_dpk_rf_backup_path_a(p_dm_odm, backup_rf_reg_addrs, ODM_RF_PATH_A, backup_rf_reg_data_a, sizeof(backup_rf_reg_addrs) / sizeof(u32));

	_dpk_init_path_a(p_dm_odm);

	if (_dpk_adjust_rf_gain_path_a(p_dm_odm)) {               /* Phase 1 */
		_dpk_gain_loss_to_find_tx_agc_path_a(p_dm_odm);          /* Phase 2 */
		if (_dpk_adjust_rf_gain_by_found_tx_agc_path_a(p_dm_odm)) /* Phase 3 */
			_dpk_do_auto_dpk_path_a(p_dm_odm);                /* Phase 4 */
	}
	_dpk_enable_dp_path_a(p_dm_odm);                         /* Phase 5 */

	_dpk_mac_bb_restore_path_a(p_dm_odm, backup_mac_bb_reg_addrs, backup_mac_bb_reg_data, sizeof(backup_mac_bb_reg_addrs) / sizeof(u32));
	_dpk_rf_restore_path_a(p_dm_odm, backup_rf_reg_addrs, ODM_RF_PATH_A, backup_rf_reg_data_a, sizeof(backup_rf_reg_addrs) / sizeof(u32));
}
...
)
{
	...
	* 舉例4, d00[10:0] = h'54c, 經過 hex2dec(h'54c) = -692,
	* 舉例5, d00[10:0] = h'7ff, 經過 hex2dec(h'7ff) = -1, */
	u32 reg0xd40_26_16 = odm_get_bb_reg(p_dm_odm, 0xD40, 0x7FF0000);
	u32 reg0xd40_10_0  = odm_get_bb_reg(p_dm_odm, 0xD40, 0x7FF);

	s32 tmp_lb_gain = _sign(reg0xd40_26_16) * _sign(reg0xd40_26_16) + _sign(reg0xd40_10_0) * _sign(reg0xd40_10_0);

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("<=== _compute_loop_back_gain_path_b, tmp_lb_gain = 0x%X\n", tmp_lb_gain));
...
)
{
	...
	u32 rf0x64_orig = odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x64, 0x7);
	u32 rf0x64_new = rf0x64_orig;

	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> _fine_tune_loop_back_gain_path_b\n"));

	do {
		/* RF setting */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, dpk_tx_agc);
		odm_write_4byte(p_dm_odm, 0x82c, 0x802083dd);
		/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
		odm_write_4byte(p_dm_odm, 0xe90, 0x0101f018);

		/* one shot */
		odm_write_4byte(p_dm_odm, 0xec8, 0x800c5599);
		odm_write_4byte(p_dm_odm, 0xec8, 0x000c5599);
		/* delay 50 ms,讓 delay 時間長一點, 確定 PA Scan function 有做完 */
		delay_ms(50);

		/* reg82c[31] = 0 --> 切到 page C */
		odm_write_4byte(p_dm_odm, 0x82c, 0x002083dd);
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, 0x33d8d);

		tmp_lb_gain = _compute_loop_back_gain_path_b(p_dm_odm);
		...
		if ((rf0x64_new == (BIT(2) | BIT(1) | BIT(0))) || (rf0x64_new == 0)) {
			...
			if (tmp_lb_gain < 263390) {/* fine tune loopback path gain: newReg64[2:0] = reg64[2:0] - 3b'001 */
				rf0x64_new -= 1;
				odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x64, 0x7, rf0x64_new);
...
)
{
	...
	/* AFE setting */
	odm_write_4byte(p_dm_odm, 0xc68, 0x59791979);
	odm_write_4byte(p_dm_odm, 0xe60, 0x77777777);
	odm_write_4byte(p_dm_odm, 0xe64, 0x77777777);

	/* external TRSW 切到 T */
	odm_write_4byte(p_dm_odm, 0xeb0, 0x77777777);
	odm_write_4byte(p_dm_odm, 0xeb4, 0x01000077);
	...
	/* r_gothrough_iqkdpk */
	odm_write_4byte(p_dm_odm, 0xe94, 0x0100005D);
	...
	/* tx_amp */
	odm_write_4byte(p_dm_odm, 0xe98, 0x41382e21);
	odm_write_4byte(p_dm_odm, 0xe9c, 0x5b554f48);
	odm_write_4byte(p_dm_odm, 0xea0, 0x6f6b6661);
	odm_write_4byte(p_dm_odm, 0xea4, 0x817d7874);
	odm_write_4byte(p_dm_odm, 0xea8, 0x908c8884);
	odm_write_4byte(p_dm_odm, 0xeac, 0x9d9a9793);
	odm_write_4byte(p_dm_odm, 0xeb0, 0xaaa7a4a1);
	odm_write_4byte(p_dm_odm, 0xeb4, 0xb6b3b0ad);

	/* tx_inverse */
	odm_write_4byte(p_dm_odm, 0xe40, 0x02ce03e9);
	odm_write_4byte(p_dm_odm, 0xe44, 0x01fd0249);
	odm_write_4byte(p_dm_odm, 0xe48, 0x01a101c9);
	odm_write_4byte(p_dm_odm, 0xe4c, 0x016a0181);
	odm_write_4byte(p_dm_odm, 0xe50, 0x01430155);
	odm_write_4byte(p_dm_odm, 0xe54, 0x01270135);
	odm_write_4byte(p_dm_odm, 0xe58, 0x0112011c);
	odm_write_4byte(p_dm_odm, 0xe5c, 0x01000108);
	odm_write_4byte(p_dm_odm, 0xe60, 0x00f100f8);
	odm_write_4byte(p_dm_odm, 0xe64, 0x00e500eb);
	odm_write_4byte(p_dm_odm, 0xe68, 0x00db00e0);
	odm_write_4byte(p_dm_odm, 0xe6c, 0x00d100d5);
	odm_write_4byte(p_dm_odm, 0xe70, 0x00c900cd);
	odm_write_4byte(p_dm_odm, 0xe74, 0x00c200c5);
	odm_write_4byte(p_dm_odm, 0xe78, 0x00bb00be);
	odm_write_4byte(p_dm_odm, 0xe7c, 0x00b500b8);
...
)
{
	...
	/* RF setting */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, 0x50bfc);
	/* Attn: A mode @ reg64[2:0], G mode @ reg56 */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x64, RFREGOFFSETMASK, 0x19aac);
	/* PGA gain: RF reg8f[14:13] */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x8a001);
	...
	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xe94, 0xf76c9f84);
	odm_write_4byte(p_dm_odm, 0xec8, 0x000c5599);
	odm_write_4byte(p_dm_odm, 0xec4, 0x11838000);
	odm_set_bb_reg(p_dm_odm, 0xed4, 0xFFF000, 0x100);  /* 將cd4[23:12] 改成 h'100, 其它位元請保留原值不要寫到
     * 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xe90, 0x0101f018);
	...
	/* read back Loopback Gain */
	odm_write_4byte(p_dm_odm, 0xeb8, 0x09000000);
	...
	/* coarse tune loopback gain by RF reg8f[14:13] = 2b'11 */
	if (tmp_lb_gain < 263390) {
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x8e001);
		_fine_tune_loop_back_gain_path_b(p_dm_odm, 0x50bfc);
...
)
{
	...
	/* dB_sel = b'101 ' 3.5 dB PA gain loss */
	/* dB_sel = b'110 ' 4.0 dB PA gain loss */
	odm_write_4byte(p_dm_odm, 0xe20, 0x00002000);

	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xe94, 0xf76c9f84);
	odm_write_4byte(p_dm_odm, 0xec8, 0x000c5599);
	odm_write_4byte(p_dm_odm, 0xec4, 0x148b8000);

	/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xe90, 0x0401f018);
	...
	/* 讀回d00[3:0] = h'a ' Tx AGC = h'1c */
	/*  */
	reg0xd40 = odm_read_4byte(p_dm_odm, 0xd40);
	switch (odm_read_4byte(p_dm_odm, 0xd40) & (BIT(3) | BIT(2) | BIT(1) | BIT(0))) {
	case 0x0:
		p_rf_calibrate_info->dpk_tx_agc = 0x50bf0 | 0x2;
		break;
	case 0x1:
...
)
{
	...
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, p_rf_calibrate_info->dpk_tx_agc);
	/* Attn: A mode @ reg64[2:0], G mode @ reg56 */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x64, RFREGOFFSETMASK, 0x19aac);
	/* PGA gain: RF reg8f[14:13] */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x8a001);
	...
	if (tmp_lb_gain < 263390) {
		/* coarse tune loopback gain by RF reg8f[14:13] = 2b'11 */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x8e001);
		_fine_tune_loop_back_gain_path_b(p_dm_odm, p_rf_calibrate_info->dpk_tx_agc);
	} else if (tmp_lb_gain > 661607) {
		/* coarse tune loopback gain by RF reg8f[14:13] = 2b'00 */
		odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x8f, RFREGOFFSETMASK, 0x88001);
		_fine_tune_loop_back_gain_path_b(p_dm_odm, p_rf_calibrate_info->dpk_tx_agc);
...
)
{
	...
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, p_rf_calibrate_info->dpk_tx_agc);
	/* Baseband data rate setting */
	odm_write_4byte(p_dm_odm, 0xe20, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe24, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe28, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe2c, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe30, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe34, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe38, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe3c, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe40, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe44, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe48, 0x3c3c3c3c);
	odm_write_4byte(p_dm_odm, 0xe4c, 0x3c3c3c3c);
	...
	/* DPK setting */
	odm_write_4byte(p_dm_odm, 0xe94, 0xf76C9f84);
	odm_write_4byte(p_dm_odm, 0xec8, 0x400C5599);
	odm_write_4byte(p_dm_odm, 0xec4, 0x11938080);    /* 0xec4[9:4]= DPk fail threshold */

	if (36 <= *(p_dm_odm->p_channel) && *(p_dm_odm->p_channel) <= 53) /* Channelchannel at low band) */
		/* r_agc */
		odm_write_4byte(p_dm_odm, 0xebc, 0x00022a1f);
	else
		/* r_agc */
		odm_write_4byte(p_dm_odm, 0xebc, 0x00009dbf);

	/* 注意page c1的c90在DPK過程中千萬不能被其他thread or process改寫成page c的c90值 */
	odm_write_4byte(p_dm_odm, 0xe90, 0x0101f018); /* TODO: 0xe90(rA_LSSIWrite_Jaguar) can not be overwritten. */

	/* one shot */
	odm_write_4byte(p_dm_odm, 0xec8, 0xc00c5599);
	odm_write_4byte(p_dm_odm, 0xec8, 0x400c5599);
	/* delay 50 ms,讓 delay 時間長一點, 確定 PA Scan function 有做完 */
	delay_ms(50);
	...
	/* T-meter RFReg42[17] = 1 to enable read T-meter, [15:10] ' T-meter value */
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x42, BIT(17), 1);
	p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_B] = odm_get_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0x42, 0xFC00);					/* 讀出42[15:10] 值並存到變數TMeter */
	dbg_print("p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_B] = 0x%X\n", p_rf_calibrate_info->dpk_thermal[ODM_RF_PATH_B]);
	odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_B, 0x00, RFREGOFFSETMASK, 0x33D8D);
	...
	/* read back */
	odm_write_4byte(p_dm_odm, 0xe90, 0x0201f01f);
	odm_write_4byte(p_dm_odm, 0xeb8, 0x0c000000);
...
)
{
	...
	/* enable IQC matrix --> 因為 BB 信號會先經過 predistortion module, 才經過 IQC matrix 到 DAC 打出去 */
	/* 所以要先 enable predistortion module {c90[7] = 1 (enable_predis) cc4[18] = 1 (確定讓 IQK/DPK module 有clock), cc8[29] = 1 (一個IQK/DPK module 裡的mux, 確認 data path 走IQK/DPK)} */
	odm_write_4byte(p_dm_odm, 0xe90, 0x0000f098);
	odm_write_4byte(p_dm_odm, 0xe94, 0x776d9f84);
	odm_write_4byte(p_dm_odm, 0xec8, 0x20000000);
	odm_write_4byte(p_dm_odm, 0xe8c, 0x3c000000);
	/* r_bnd */
	odm_write_4byte(p_dm_odm, 0xeb8, 0x000fffff);

	if (p_rf_calibrate_info->is_dpk_fail) {
		/* cc4[29] = 1 (bypass DP LUT) */
		odm_write_4byte(p_dm_odm, 0xe98, 0x40004000);
		odm_write_4byte(p_dm_odm, 0xec4, 0x28840000);
	} else {
		odm_write_4byte(p_dm_odm, 0xe98, p_rf_calibrate_info->dpk_gain);
		odm_write_4byte(p_dm_odm, 0xec4, 0x08840000);

		/* PWSF */
		/* 寫PWSF table in 1st SRAM for PA = 11 use */
		odm_write_4byte(p_dm_odm, 0xe20, 0x00000800);
...

		{
			...
			for (i = start_idx, j = 0; (center_addr - j + 1) >= 1; i -= 2, j++) {
				...
				odm_write_4byte(p_dm_odm, 0xee4, value);
				odm_write_1byte(p_dm_odm, 0xee4, 0x0);		/* write disable */
			}
...
)
{
	...
	_dpk_mac_bb_backup_path_b(p_dm_odm, backup_mac_bb_reg_addrs, backup_mac_bb_reg_data, sizeof(backup_mac_bb_reg_addrs) / sizeof(u32));
	_dpk_rf_backup_path_b(p_dm_odm, backup_rf_reg_addrs, ODM_RF_PATH_B, backup_rf_reg_data_a, sizeof(backup_rf_reg_addrs) / sizeof(u32));

	_dpk_init_path_b(p_dm_odm);

	if (_dpk_adjust_rf_gain_path_b(p_dm_odm)) {               /* Phase 1 */
		_dpk_gain_loss_to_find_tx_agc_path_b(p_dm_odm);          /* Phase 2 */
		if (_dpk_adjust_rf_gain_by_found_tx_agc_path_b(p_dm_odm)) /* Phase 3 */
			_dpk_do_auto_dpk_path_b(p_dm_odm);                /* Phase 4 */
	}
	_dpk_enable_dp_path_b(p_dm_odm);                         /* Phase 5 */

	_dpk_mac_bb_restore_path_b(p_dm_odm, backup_mac_bb_reg_addrs, backup_mac_bb_reg_data, sizeof(backup_mac_bb_reg_addrs) / sizeof(u32));
	_dpk_rf_restore_path_b(p_dm_odm, backup_rf_reg_addrs, ODM_RF_PATH_B, backup_rf_reg_data_a, sizeof(backup_rf_reg_addrs) / sizeof(u32));
}
void
...
{
	p_dm_odm->dpk_done = true;
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> phy_dp_calibrate_8812a\n"));
	_phy_dp_calibrate_path_a_8812a(p_dm_odm);
	_phy_dp_calibrate_path_b_8812a(p_dm_odm);
	ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("<=== phy_dp_calibrate_8812a\n"));
}
