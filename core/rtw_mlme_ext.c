...
_continue:
			...
			pmlmepriv->cur_network.join_res == true)
	{
		...
		if (psta == NULL)
		{
			...
			psta->htpriv.bwmode = CHANNEL_WIDTH_20;
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	*(fctrl) = 0;
	...
	// DS parameter set
#ifdef CONFIG_IOCTL_CFG80211
	if(wdev_to_priv(padapter->rtw_wdev)->p2p_enabled && listen_channel !=0 && pwdinfo->driver_interface == DRIVER_CFG80211 )
	{
...
#ifdef CONFIG_AUTO_AP_MODE
{
	...
	if (psta && psta->isrc && psta->pid>0)
	{
		...
		u8 RC_INFO[14] = {0};
		//EID[1] + EID_LEN[1] + RC_OUI[4] + MAC[6] + PairingID[2] + ChannelNum[2]
		u16 cu_ch = le16_to_cpu(cur_network->Configuration.DSConfig)
	...
	*(fctrl) = 0;
	...
	if(padapter->mlmepriv.htpriv.ht_option==true)
	{
		...
		if ((p != NULL) && (!(is_ap_in_tkip(padapter))))
		{
			...
			HT_caps_info = le16_to_cpu(pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info);
			...
			HT_caps_info |= 0x000c;
			pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info = cpu_to_le16(HT_caps_info);
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	le16_to_cpup((__le16 *)&bssid->Configuration.BeaconPeriod);
...
void _linked_info_dump(_adapter *padapter)
{
	...
	//============  rx info ============
	rtw_hal_set_def_var(padapter, HW_DEF_FA_CNT_DUMP,&mac_id);
...
//#endif
	...
	if (padapter->registrypriv.wifi_spec) {
		...
		if (padapter->mlmepriv.htpriv.ht_option == false){
			acparm = 0x00004320;
			rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_BE, (u8 *)(&acparm));
		}
	}
...
int concurrent_chk_start_clnt_join(_adapter *padapter)
{
		...
		check_fwstate(pbuddy_mlmepriv, WIFI_STATION_STATE) == true) //for Client Mode/p2p client
	{
		...
		struct wifidirect_info *pbuddy_wdinfo = &(pbuddy_adapter->wdinfo);
		if(!rtw_p2p_chk_state(pbuddy_wdinfo, P2P_STATE_NONE) && pbuddy_wdinfo->driver_interface == DRIVER_CFG80211 )
		{
