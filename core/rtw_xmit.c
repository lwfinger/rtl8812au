...
s32	_rtw_init_xmit_priv(struct xmit_priv *pxmitpriv, _adapter *padapter)
{
	...
	*/
	...
	if (pxmitbuf) {
		...
		if((res=rtw_os_xmit_resource_alloc(padapter, pxmitbuf, 0, true)) == _FAIL) {
			res= _FAIL;
			goto exit;
		}
	}
...
void _rtw_free_xmit_priv (struct xmit_priv *pxmitpriv)
{
	...
	for(i=0; i<num_xmit_extbuf; i++)
	{
		rtw_os_xmit_resource_free(padapter, pxmitbuf,(max_xmit_extbuf_size + XMITBUF_ALIGN_SZ), true);
	...
	pxmitbuf = &pxmitpriv->pcmd_xmitbuf;
	rtw_os_xmit_resource_free(padapter, pxmitbuf, 0, true);
...
*/
	...
	else
	{
		while (true)
		{
			...
			if((pattrib->ampdu_en==true) && (!IS_HARDWARE_TYPE_JAGUAR(padapter)))
			{
				pattrib->vcs_mode = RTS_CTS;
				break;
			}
...
struct xmit_buf *rtw_alloc_cmd_xmitbuf(struct xmit_priv *pxmitpriv, u32 buffsize)
{
	...
	pxmitbuf = &pxmitpriv->pcmd_xmitbuf;
	if (pxmitbuf !=  NULL) {
		if(rtw_os_xmit_resource_alloc(pxmitpriv->adapter, pxmitbuf,(buffsize + XMITBUF_ALIGN_SZ), false) == _FAIL) {
			return NULL;
		}
...
s32	rtw_free_cmd_xmitbuf(struct xmit_priv *pxmitpriv)
{
	...
	if (pxmitbuf==NULL) {
		...
		rtw_os_xmit_resource_free(pxmitbuf->padapter, pxmitbuf, pxmitbuf->alloc_sz, false);
	}
